...
SHT3x_Status SHT3x_CreateDefaultSensor(SHT3x_Sensor *s) {
    ...
    s->mode                  = SHT3X_MODE_PERIODIC;
    s->config->i2c_address   = SHT3X_I2C_ADDR_VSS;
    s->config->repeatability = SHT3X_REPEAT_HIGH;
    s->config->meas_per_sec  = SHT3X_MPS_1;
    s->config->clock_stretch = false;
...
static bool _sht3x_is_valid(SHT3x_Sensor *s)
{
    ...
    if (s->mode != SHT3X_MODE_SINGLE_SHOT && s->mode != SHT3X_MODE_PERIODIC) {
        return false;
    }
    ...
    if (s->mode == SHT3X_MODE_PERIODIC && s->config->meas_per_sec > SHT3X_MPS_10) {
        return false;
    }
    return true;
}
...
SHT3x_Status SHT3x_Read(SHT3x_Sensor *s, SHT3x_Data *out)
{
    ...
    switch (s->mode) {
        case SHT3X_MODE_SINGLE_SHOT: {
            s = _sht3x_send_cmd(s, _sht3x_singleshot_cmd(s->config->repeatability, s->config->clock_stretch));
            if (s != SHT3X_OK) {
                return s;
            }
            if (!s->config->clock_stretch) {
                s->hal->delay_ms(_sht3x_meas_delay_ms(s->config->repeatability));
            }
            break;
        }
        case SHT3X_MODE_PERIODIC: {
...
SHT3x_Status SHT3x_Deinit(SHT3x_Sensor *s)
{
    ...
    SHT3x_Status s = SHT3X_OK;

    if (dev->initialized
        && dev->mode == SHT3X_MODE_PERIODIC
        && dev->periodic_running)
    {
        s = _sht3x_send_cmd(dev, SHT3X_CMD_BREAK);
